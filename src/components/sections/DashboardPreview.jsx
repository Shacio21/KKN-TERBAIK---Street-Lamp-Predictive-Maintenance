...
import { energyChartData as fallbackEnergyChartData, lampStatuses as fallbackLampStatuses } from '../../data/mockData';
...
function StatusBadge({ status }) {
  const styles = {
    ...
    fault: { bg: 'rgba(239,68,68,0.1)', color: '#EF4444', text: 'Fault' },
    maintenance: { bg: 'rgba(139,92,246,0.1)', color: '#8B5CF6', text: 'Maintenance' },
  };
...
function EnergyChart({ data }) {
  const maxVal = Math.max(...data.map(d => Math.max(d.generated, d.consumed)), 1);
...
export default function DashboardPreview({ lampStatuses = fallbackLampStatuses, energyChartData = fallbackEnergyChartData, kpi = null, isLive = false }) {
  ...
  const avgBattery = kpi?.avg_battery_level ?? Math.round(lampStatuses.reduce((sum, lamp) => sum + lamp.battery, 0) / Math.max(lampStatuses.length, 1));
  const onlineRate = kpi?.total_lamps ? Math.round((kpi.online_lamps / kpi.total_lamps) * 1000) / 10 : Math.round((onlineCount / Math.max(lampStatuses.length, 1)) * 1000) / 10;
        ...
        >
          ...
          <div className="flex items-center justify-between mb-6 pb-4 border-b border-border">
            <div className="flex items-center gap-3">
              <div className={`w-3 h-3 rounded-full ${isLive ? 'bg-neon-green' : 'bg-neon-amber'} animate-[neon-pulse_2s_ease-in-out_infinite]`} />
            ...
            <div className="space-y-4">
              ...
              <GlassCard hover={false} className="p-4">
                ...
                <div className="space-y-3">
                  {[
                    { icon: Battery, label: 'Avg Battery', value: `${avgBattery}%`, color: '#00FF88' },
                    { icon: SunIcon, label: 'Solar Status', value: 'Charging', color: '#F59E0B' },
                    { icon: Wifi, label: 'Network', value: `${onlineRate}% Online`, color: '#00D4FF' },
                    { icon: AlertTriangle, label: 'Active Alerts', value: `${kpi?.unresolved_alerts ?? 1}`, color: '#F59E0B' },
